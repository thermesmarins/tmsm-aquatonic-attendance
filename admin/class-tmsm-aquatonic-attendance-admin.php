 ...
 * @subpackage Tmsm_Aquatonic_Attendance/admin
 */
 ...
 */
class Tmsm_Aquatonic_Attendance_Admin {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/tmsm-aquatonic-attendance-admin.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/tmsm-aquatonic-attendance-admin.js', array( 'jquery' ), $this->version, false );
	...
	function test_cron_schedule_exists( $tests ) {
		$tests['direct']['tmsm_aquatonic_attendance'] = array(
			'label' => __('TMSM Aquatonic Attendance Cron Schedule Exists', 'tmsm-aquatonic-attendance'),
			'test'  => 'tmsm_aquatonic_attendance_test_schedule_exists',
		);
		return $tests;
	}
	...
	public function options_page_menu() {
		add_options_page( __('Aquatonic Attendance', 'tmsm-aquatonic-attendance'), __('Aquatonic Attendance', 'tmsm-aquatonic-attendance'), 'manage_options', $this->plugin_name.'-settings', array($this, 'options_page_display'));
	...
	function settings_link( $links ) {
		$setting_link = array(
			'<a href="' . admin_url( 'options-general.php?page='.$this->plugin_name.'-settings' ) . '">'.__('Settings', 'tmsm-aquatonic-attendance').'</a>',
		);
	...
	public function register_fields() {
		...
		add_settings_field(
			'webservicecounturl',
			esc_html__( 'Web Service Count URL', 'tmsm-aquatonic-attendance' ),
		...
		for ($tier = 1; $tier <= 5; $tier++) {
			add_settings_field(
				"tier{$tier}_value",
				esc_html( sprintf( __( 'Tier %s Value', 'tmsm-aquatonic-attendance' ), $tier ) ),
				array( $this, 'field_text' ),
				$this->plugin_name,
			...
			add_settings_field(
				"tier{$tier}_color",
				esc_html( sprintf( __( 'Tier %s Color', 'tmsm-aquatonic-attendance' ), $tier ) ),
				array( $this, 'field_text' ),
				$this->plugin_name,
				$this->plugin_name . '-tiers',
				array(
					...
					'id' => "tier{$tier}_color",
					//'value' 		=> 'Thank you for your interest! There are no job openings at this time.',
					'description' => esc_html__( 'Hexadecimal', 'tmsm-aquatonic-attendance' ),
				)
			);

			add_settings_field(
				"tier{$tier}_shortdesc",
				esc_html( sprintf( __( 'Tier %s Short Description', 'tmsm-aquatonic-attendance' ), $tier ) ),
				array( $this, 'field_text' ),
				$this->plugin_name,
				$this->plugin_name . '-tiers',
				array(
			...
			add_settings_field(
				"tier{$tier}_longdesc",
				esc_html( sprintf( __( 'Tier %s Long Description', 'tmsm-aquatonic-attendance' ), $tier ) ),
	...
	public function register_sections() {
		...
		add_settings_section(
			$this->plugin_name . '-webservice',
			esc_html__( 'Web Service', 'tmsm-aquatonic-attendance' ),
		...
		add_settings_section(
			$this->plugin_name . '-tiers',
			esc_html__( 'Tiers', 'tmsm-aquatonic-attendance' ),
		...
		add_settings_section(
			...
			esc_html__( 'Time Slots', 'tmsm-aquatonic-attendance' ),
	...
	private function sanitizer( $type, $data ) {
		...
		$return 	= '';
		$sanitizer 	= new Tmsm_Aquatonic_Attendance_Sanitize();
 ...
 * @return array
 */
function tmsm_aquatonic_attendance_test_schedule_exists() {
	$result = array(
		'label'       => __('TMSM Aquatonic Attendance Cron Schedule Exists', 'tmsm-aquatonic-attendance'),
		'status'      => 'good',
		'badge'       => array(
		...
		'description' => sprintf(
			'<p>%s</p>',
			__( 'Cron Schedule is not fired', 'tmsm-aquatonic-attendance' )
		),
		'actions'     => '',
		'test'        => 'tmsm-aquatonic-attendance',
	);

	if ( ! wp_next_scheduled( 'tmsm_aquatonic_attendance_cronaction' ) ) {
		$result['status'] = 'critical';
		$result['badge']['color'] = 'red';
		$result['label'] = __('TMSM Aquatonic Attendance Cron Schedule is not added', 'tmsm-aquatonic-attendance');
		$result['description'] = sprintf(
			'<p>%s</p>',
			__('Cron schedule is not fired, prices will not update from Aquatonic Attendance.', 'tmsm-aquatonic-attendance')
		);
		$result['actions'] .= sprintf(
			'<p><a href="%s">%s</a></p>',
			esc_url( admin_url( 'plugins.php' ) ),
			__( 'Disable and enable again the plugin', 'tmsm-aquatonic-attendance' )
		);
	}
